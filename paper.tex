...
\subsection{Setup}
\textbf{Model Configuration:} 7B parameters total (3.2B Transformer, 2.8B Mamba, 1B RBM+bridges). Hidden dimensions: $D_t = 4096$, $D_s = 2048$, $D_r = 1024$. Gibbs steps $K = 8$.

\textbf{Training:} 1.2T tokens from The Pile (800B) and CodeSearchNet (400B). AdamW optimizer ($\beta_1=0.9$, $\beta_2=0.95$), learning rate 1.5e-4 with cosine decay, batch size 4M tokens.

\textbf{Hardware:} Google Cloud TPUv6 Trillium Pod (256 chips, 32GB HBM3 each).
...
\centering
\small
\caption{Zero-shot benchmark comparison (Accuracy \%)}
\label{tab:main}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Benchmark} & \textbf{Llama-2} & \textbf{Llama-3} & \textbf{Mamba} & \textbf{Ours} \\
\midrule
ARC-Challenge & 53.0 & 54.3 & 49.8 & \textbf{59.4} \\
GSM8K & 69.5 & 72.0 & 68.5 & \textbf{74.8} \\
HumanEval & 65.2 & 68.4 & 61.2 & \textbf{71.8} \\
MMLU & 64.0 & \textbf{66.0} & 62.5 & 65.8 \\
BIG-Bench Hard & 45.3 & 47.1 & 43.2 & \textbf{50.2} \\
\bottomrule
\end{tabular}
\end{table}

T-M-B-M-T shows consistent improvements on reasoning-intensive benchmarks (ARC-Challenge, GSM8K, BIG-Bench Hard) while matching Llama-3 on knowledge-intensive tasks (MMLU). The largest gains appear on multi-step reasoning, suggesting the RBM deliberation phase provides genuine benefit.
...
\centering
\small
\caption{Ablation study on ARC-Challenge}
\label{tab:ablation}
\begin{tabular}{@{}lcc@{}}
\toprule
...
T-M-B-M-T (Full) & 59.4 & -- \\
\midrule
No RBM (T-M-M-T) & 51.2 & -8.2 \\
No Mamba (T-B-T) & 54.8 & -4.6 \\
No Spectral Norm & 38.4 & -21.0 \\
Fewer Gibbs steps ($K=2$) & 55.1 & -4.3 \\
More Gibbs steps ($K=16$) & 59.6 & +0.2 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*,noitemsep,topsep=2pt]
    \item Removing the RBM core (T-M-M-T) causes the largest performance drop, confirming the value of energy-based deliberation
    \item Spectral normalization is critical: without it, training becomes unstable
    \item $K=8$ Gibbs steps is near-optimal; more steps yield diminishing returns
\end{itemize}
